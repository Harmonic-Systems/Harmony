...
def get_nested_chat_trigger_agent_names(
    ...
    for message in nested_chat.triggered_by:
        harmony_chat = next(chat for chat in all_chats if chat.id == message.id)
        if message.is_reply:
            trigger_agent_ids.append(harmony_chat.target)
        else:
            trigger_agent_ids.append(harmony_chat.source)
...
def get_nested_chat_message_string(
    ...
    sender_id = harmony_chat.target if message.is_reply else harmony_chat.source
    recipient_id = (
        harmony_chat.source if message.is_reply else harmony_chat.target
    )
    if sender_id != agent.id:
        sender_name = agent_names[sender_id]
    recipient_name = agent_names[recipient_id]
    chat_dict = harmony_chat.get_chat_args()
    chat_dict["recipient"] = recipient_name
    if sender_name:
        chat_dict["sender"] = sender_name
    message_value, message_source = get_chat_nested_string(
        chat=harmony_chat, is_reply=message.is_reply, chat_names=chat_names
    )
