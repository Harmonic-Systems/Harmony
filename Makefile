...
help:
	...
	@echo " export           Export *.harmony files in ./examples to {.py,.ipynb}"
...
docs-live:
	...
	python -m mkdocs serve --watch mkdocs.yml --watch docs --watch harmony --dev-addr localhost:8400
...
.PHONY: install
install: build
	python -m pip uninstall -y harmony
